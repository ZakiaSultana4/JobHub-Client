...
const All = () => {
  ...
  console.log(jobs); 
   useEffect(() => {
    axiosSecure
      .get(`/all-jobs?page=${currentPage}&size=${itemsPerPage}&filter=${filter}&sort=${sort}&search=${search}`)
      .then((res) => {
        setJobs(res.data);
  ...
  useEffect(() => {
    const getCount = async () => {
      const { data } = await axios(
        `https://jobhub-a11.vercel.app/jobs-count?filter=${filter}&search=${search}`
      )
  ...
  console.log(search);
  return (
    <div className="container px-5 py-10 mx-auto min-h-[calc(100vh-306px)] flex flex-col justify-between">
      <div>
